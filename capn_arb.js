...
class CapnRab{
		...
		return (async () => {
			...
			////////////////////////////////
		   	...
		   	// for(let i=0;i<RECONNECT.retry_max; i++){
				...
				this.web3    = new Web3(this.config.RPC_URL, options);
	...
	async withdraw_erc20_from_capn(symbol, amount){
		...
		const send_params = await this.get_send_params({"multiply_gas" : 10});
	...
	async swap(name, stats=[], min_expected){
		...
		let tx_count = await this.web3.eth.getTransactionCount(this.account.address);
		...
		for(let i=0;i<stats.length; i++){
	        ...
	        // - gas multiplier 3, gasPrice multiplier 2
	        //
	        // 0.23 + 0.34 + 0.26
			...
			// let gas = (new BigNumber(stats[i].quote.gas)).multipliedBy(           3 ); 
			// let gasPrice = (new BigNumber(stats[i].quote.gasPrice)).multipliedBy( 2 );	

			const gas 		= parseInt(stats[i].quote.gas) 		* 3;
			const gasPrice 	= parseInt(stats[i].quote.gasPrice);	
			...
			// logger.log(`adjusted   | gas: ${(gas.toString())} | gasPrice: ${gasPrice.toString()}`)
			logger.log(`adjusted   | gas: ${gas} | gasPrice: ${gasPrice}`)
