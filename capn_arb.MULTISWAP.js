...
class CapnRab{
	...
	async check_rpc(){
		...
		logger.log("Checking RPC");
		logger.log(this.web3==null);
		if (!await this.web3.eth.net.isListening()) {

			const newProvider = new Web3(this.config.RPC_URL, {
				reconnect: {
					auto: true,
					delay: 1000, // ms
					maxAttempts: 5,
		...
		setTimeout(this.check_rpc, 5000);
	}
	...
	async get_send_params(options={"multiply_gas" : null}){
		...
		const gasPrice 		= new BigNumber(await this.get_gas_price());
		if(this.chain=="ropsten"){
	...
	async swap(name, stats=[], min_expected){
		...
		const bal_before = await this.get_balance({
			"human_digits" : true, 
			"non_zero_only": true
		});
		...
		// console.log("===========> msg_values <=============");
		// console.log(msg_values);

		// console.log("===========> sum_msg_values <=============");
		// console.log(sum_msg_values);

		// console.log("===========> terminal_coin <=============");
		// console.log(terminal_coin);	    
	    ...
	    const gas_prices = stats.map(s => s.quote.gasPrice);
	    
	    let max_gas_price= BigNumber.max.apply(null, gas_prices);
	    max_gas_price = max_gas_price.multipliedBy(2);
	    
	    let gas = new BigNumber(gas_combined);
	    gas = gas.multipliedBy(3);

	    const send_params = {
            from 	: this.account.address,
            // value 	: stats[0].quote.value,
            value 	: sum_msg_values,
            gas 	: gas.toFixed(),
            gasPrice: max_gas_price.toFixed(),
        };
        ...
        try {
        	...
        	console.log(banner("❌ Transaction Failed ❌"));
        	logger.log(err);
	...
	async test_single_swap(options={
		// "sellToken" : "WETH", "buyToken": "DAI", "sellAmount" : "10000000000000000"
		"sellToken" : "DAI", "buyToken"	: "WETH","sellAmount" : "4055699900084213083"
	}) {
...
async function get_capn_instance(){
	if(!capn){
		capn = await new CapnRab();
	}
	return capn;
}
...
async function main(){
    ...
    const capn  = await get_capn_instance();
